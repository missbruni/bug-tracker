...
function Layout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const fetchCount = async () => {
      const { count } = await sb.from('bugs').select('*', { count: 'exact', head: true }).eq('reviewed', false)
      if (count !== null) setActiveBugCount(count)
    }
    fetchCount()
    const channel = sb.channel('layout-bugs-count')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'bugs' }, () => fetchCount())
      .subscribe()
  ...
  const toggleBugs = () => setShowBugs(prev => { const next = !prev; localStorage.setItem('showBugs', String(next)); return next })

  return (
    <div className="min-h-screen bg-slate-50 dark:bg-gray-950 font-sans">
      <NavBar showBugs={showBugs} onToggleBugs={toggleBugs} bugCount={activeBugCount}><ThemeToggle /></NavBar>
      {children}
    </div>
  )
}
