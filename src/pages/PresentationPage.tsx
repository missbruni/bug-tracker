...
export default function PresentationPage() {
  ...
  React.useEffect(() => {
    const load = async () => {
      ...
      const testerMap = new Map(allTesters.map(tester => [tester.id, tester]))
      const assignMap = new Map(assigns.map(assignment => [assignment.scenario_id, testerMap.get(assignment.tester_id) || null]))
      ...
      const builtSlides: Slide[] = [
        ...
        ...scenarios.map(scenario => ({
          ...
          scenario,
          assignedTester: assignMap.get(scenario.id) || null,
        })),
        { type: 'crosscutting' },
        { type: 'edgecases' },
        { type: 'bugreporting' },
  ...
  React.useEffect(() => {
    const handler = (event: KeyboardEvent) => {
      if (event.key === 'ArrowRight' || event.key === ' ') {
        ...
        setCurrent(currentIndex => Math.min(currentIndex + 1, slides.length - 1))
      }
      if (event.key === 'ArrowLeft') {
        event.preventDefault()
        setCurrent(currentIndex => Math.max(currentIndex - 1, 0))
      }
      if (event.key === 'Escape') {
  ...
  const scenarioSlides = slides.filter(slideItem => slideItem.type === 'scenario')
  const totalTesters = new Set(scenarioSlides.filter(slideItem => slideItem.assignedTester).map(slideItem => slideItem.assignedTester!.id)).size

  return (
    <div className="min-h-screen bg-gray-950 text-gray-300 font-sans select-none">
      {/* Navigation overlay */}
      <div className="fixed inset-0 z-10 flex">
        <div className="w-1/3 cursor-pointer" onClick={() => setCurrent(currentIndex => Math.max(currentIndex - 1, 0))} />
        <div className="w-1/3" />
        <div className="w-1/3 cursor-pointer" onClick={() => setCurrent(currentIndex => Math.min(currentIndex + 1, slides.length - 1))} />
      </div>
      ...
      <div className="fixed bottom-4 left-0 right-0 z-20 flex items-center justify-center gap-4">
        <button onClick={() => setCurrent(currentIndex => Math.max(currentIndex - 1, 0))} disabled={current === 0}
        ...
        <button onClick={() => setCurrent(currentIndex => Math.min(currentIndex + 1, slides.length - 1))} disabled={current === slides.length - 1}
          className="p-2 rounded-full bg-gray-800 border border-gray-700 text-gray-400 hover:text-white disabled:opacity-30 cursor-pointer disabled:cursor-default transition-colors">
          ...
          {slide.type === 'objective' && (
            <div>
              ...
              <div className="flex items-center gap-1 flex-wrap mb-6">
                {['Home', 'Search', 'Availabilities', 'Room Detail', 'Add-ons', 'Summary', 'Payment'].map((step, stepIndex) => (
                  <span key={step}>
                    {stepIndex > 0 && <span className="text-gray-600 mx-1">→</span>}
          ...
          {slide.type === 'assignments' && (
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {scenarioSlides.map(scenarioSlide => (
                  <div key={scenarioSlide.scenario!.id} className="bg-gray-900 border border-gray-700 rounded-lg p-3 relative">
                    <div className="text-blue-400 font-bold text-sm mb-0.5">Tester {scenarioSlide.scenario!.letter}</div>
                    <div className="text-gray-500 text-xs mb-1">{scenarioSlide.scenario!.title}</div>
                    {scenarioSlide.scenario!.device_requirement && (
                      <div className="text-amber-400 text-xs font-medium mb-1">{scenarioSlide.scenario!.device_requirement}</div>
                    )}
                    {scenarioSlide.assignedTester ? (
                      <span className="inline-block bg-blue-500 text-gray-950 font-bold px-2 py-0.5 rounded-full text-xs mt-1">
                        {scenarioSlide.assignedTester.name}
                      </span>
                    ) : (
            ...
            <div>
              ...
              <div className="bg-gray-900 border border-gray-700 rounded-xl p-6">
                <div className="space-y-2">
                  {(slide.scenario.description || '').split('\n').filter(lineText => lineText.trim()).map((line, lineIndex) => {
                    const isStep = /^\d+\./.test(line.trim())
                    const isPms = line.startsWith('✓ PMS')
                    if (isPms) {
                      return (
                        <div key={lineIndex} className="mt-4 bg-green-900/20 border border-green-700/30 rounded-lg p-3 text-sm">
                    ...
                    return <p key={lineIndex} className="text-sm text-gray-400">{line}</p>
                  })}
                </div>
              </div>
            </div>
          ...
          {slide.type === 'edgecases' && (
            <div>
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-2">
                {EDGE_CASES.map((edgeCase, edgeCaseIndex) => (
                  <div key={edgeCaseIndex} className="flex items-center gap-2 text-sm text-gray-400">
                    <span className="text-gray-600">☐</span> {edgeCase}
                  </div>
                ))}
              </div>
            </div>
