  ...
  return (
    <>
      <div className="sticky top-0 z-40 bg-white/90 dark:bg-gray-900/90 backdrop-blur-md border-b border-slate-200 dark:border-gray-800/50 text-slate-900 dark:text-white">
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-7 py-3 flex flex-wrap items-center justify-between gap-x-4 sm:gap-x-8 gap-y-2">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto h-9">
            <div className="relative h-full flex-1 sm:flex-none">
              ...
              <input
                ...
                className={`h-full w-full sm:w-64 rounded-lg border border-slate-300 dark:border-slate-600 bg-slate-100 dark:bg-slate-800/60 pl-9 ${showSearchShortcut ? 'pr-16' : 'pr-3'} text-xs text-slate-900 dark:text-white outline-none focus:border-blue-400 dark:focus:border-blue-400 focus:ring-1 focus:ring-blue-200 dark:focus:ring-blue-400/30 placeholder:text-slate-400 dark:placeholder:text-slate-500 transition-all`}
              />
              {showSearchShortcut && (
                <kbd className="absolute right-2 top-1/2 -translate-y-1/2 hidden sm:flex items-center gap-1 rounded bg-slate-200 dark:bg-slate-700 px-1.5 py-0.5 text-[10px] text-slate-500 dark:text-slate-300 font-mono pointer-events-none">⌘ K</kbd>
              )}
            </div>
            {actionButton}
          </div>
