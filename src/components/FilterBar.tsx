...
export default function FilterBar({
  ...
  return (
    <div className="border-b border-slate-200 dark:border-gray-800">
      ...
      <div className="hidden sm:flex max-w-screen-2xl mx-auto flex-wrap items-center gap-2 px-7 py-3.5">
        {renderFilters()}
      </div>
      ...
      {sheetOpen && (
        <div className="sm:hidden fixed inset-0 z-50">
          <div className={`absolute inset-0 transition-opacity duration-200 ${closing ? 'opacity-0' : 'opacity-100'} bg-black/40`} onClick={closeSheet} />
          <div className="absolute bottom-0 left-0 right-0 bg-white dark:bg-gray-900 rounded-t-2xl p-5 pb-8 max-h-[80vh] overflow-y-auto" style={{ animation: `${closing ? 'slideDown' : 'slideUp'} 0.25s ease-out forwards` }}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-slate-900 dark:text-gray-100">Filters</h3>
              <button
                onClick={closeSheet}
            ...
            <div className="space-y-4">
              {/* Severity */}
              <div>
                <label className="block text-[11px] font-semibold text-slate-500 dark:text-gray-400 uppercase mb-2">Status / Severity</label>
              ...
              {/* Tester */}
              <div>
                <label className="block text-[11px] font-semibold text-slate-500 dark:text-gray-400 uppercase mb-2">Tester</label>
                <select
                  value={testerFilter}
                  onChange={(event) => setTesterFilter(event.target.value)}
              ...
              {/* Date */}
              <div>
                <label className="block text-[11px] font-semibold text-slate-500 dark:text-gray-400 uppercase mb-2">Date</label>
                <select
                  value={dateFilter}
                  onChange={(event) => setDateFilter(event.target.value)}
                  className="w-full rounded-md border border-slate-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-2 text-xs text-slate-600 dark:text-gray-400"
              ...
              {sessions.length > 0 && (
                <div>
                  <label className="block text-[11px] font-semibold text-slate-500 dark:text-gray-400 uppercase mb-2">Session</label>
                  <select
                    value={sessionFilter}
                    onChange={(event) => setSessionFilter(event.target.value)}
                    className="w-full rounded-md border border-slate-300 dark:border-gray-700 bg-white dark:bg-gray-800 px-3 py-2 text-xs text-slate-600 dark:text-gray-400"
              ...
              {/* Sort */}
              <div>
                <label className="block text-[11px] font-semibold text-slate-500 dark:text-gray-400 uppercase mb-2">Sort</label>
                <button
                  onClick={() => setSortOrder(sortOrder === 'newest' ? 'oldest' : sortOrder === 'oldest' ? 'default' : 'newest')}
                ...
                >
                  <ArrowDownUp size={12} />
                  {sortOrder === 'newest' ? 'Newest first' : sortOrder === 'oldest' ? 'Oldest first' : 'Default order'}
                </button>
              </div>
            </div>
